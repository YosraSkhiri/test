...
const Favorite = ({postId}: FavoriteProps) => {
  const { addFavorite, removeFavorite, favoriteExist } = useFavorites()
  const [isFavorite, setFavorite] = useState<'loading' | 'not_favorite' | 'favorite'>('loading')

  useEffect(() => {
    if (!favoriteExist(postId)) {
      setFavorite('not_favorite')
    } else {
      setFavorite('favorite')
    }
  }, [favoriteExist, postId])
